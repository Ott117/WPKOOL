   ...
   if (isset($_REQUEST["gallery_id"])) {
      if ($id == intval($_REQUEST["album_id"])) {
         ...
         $gallery_data = $wpdb->get_row(
             $wpdb->prepare("SELECT * FROM " . gallery_master_meta() . " WHERE meta_key = %s AND old_gallery_id = %d", "gallery_data", $id)
         );
      }
   } else {
      $gallery_data = $wpdb->get_row(
   ...
   if (isset($gallery_data) && count($gallery_data) > 0) {
      ...
      if (isset($display) && $display == "selected") {
         ...
         $manage_data = $wpdb->get_results
             ...
             $wpdb->prepare
                 (
                 "SELECT * FROM " . gallery_master_meta() . " WHERE old_gallery_id = %d AND meta_key != %s ORDER BY meta_id DESC LIMIT $images_count", $id, "gallery_data"
             )
         );
