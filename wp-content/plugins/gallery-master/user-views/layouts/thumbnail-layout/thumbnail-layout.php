      ...
      if (count($gallery_image_detail_only_included_images) > 0) {
         foreach ($gallery_image_detail_only_included_images as $pic) {
            if ($pic["file_type"] == "image") {
               $filename_thumbs = GALLERY_MASTER_THUMBS_CROPPED_URL . $pic["image_name"];
               ...
               if (!file_exists(GALLERY_MASTER_THUMBS_NON_CROPPED_DIR . $pic["image_name"])) {
                  if (strpos($pic["image_name"], ".") !== false) {
                     ...
                     $filename_thumbs = GALLERY_MASTER_THUMBS_CROPPED_URL . $filename_actual[0] . "." . strtoupper($filename_actual[1]);
            ...
            ?>
            <div id="grid_wrapper_item_<?php echo $random . "_" . $pic["id"]; ?>" class="grid_wrapper_item gm_animate"  data-animate="<?php echo isset($animation_effects) ? $animation_effects : 'none'; ?>" data-duration="1.0s" data-delay="0.1s" data-offset="100">
               <?php
                        ...
                        case "foo_box_free_edition":
                           if ($pic["image_title"] == "" && $pic["image_description"] == "") {
                              ...
                              <div id="grid_item_image_<?php echo $random . "_" . $pic["id"]; ?>" class="grid_item_image_<?php echo $random; ?> <?php echo $pic["file_type"] == "video" ? 'grid_item_video_' . $random : ""; ?>" style="background:url('<?php echo $pic["file_type"] == "image" ? $filename_thumbs : esc_attr($pic["video_thumb"]); ?>');background-size:100% 100% !important;">
                                 <img src="<?php echo $pic["file_type"] == "image" ? $filename_thumbs : esc_attr($pic["image_name"]); ?>" image_full_path="<?php echo $pic["file_type"] ?>" alt="<?php echo esc_attr($pic["alt_text"]); ?>" id="ux_gm_file" name= "ux_gm_file_<?php echo $random . "_" . $pic["id"]; ?>"/>
                              </div>
                           </a>
                           <?php
                           if ($pic["image_title"] != "" || $pic["image_description"] != "") {
