...
if (!is_user_logged_in()) {
   ...
   if (!$access_granted) {
         ...
         $parent_id = $wpdb->get_var
             ...
             $wpdb->prepare
                 ...
                 "SELECT id FROM " . gallery_master_parent() . "
						WHERE " . gallery_master_parent() . ". type = %s", $type
             )
         );
         return $parent_id;
      }
      function get_delete_all_galleries_module_gallery_master($type) {
         global $wpdb;
         $get_all_galleries_and_image = $wpdb->get_results
             (
             $wpdb->prepare
                 (
                 "SELECT meta_id FROM " . gallery_master_meta() . "
						INNER JOIN " . gallery_master_parent() . " ON " . gallery_master_meta() . ".meta_id = " . gallery_master_parent() . ".id
						WHERE " . gallery_master_parent() . ".type = %s", $type
             )
         );
      ...
      function get_dir_contents_gallery_master($dir, &$results = array()) {
         $files = scandir($dir);
         foreach ($files as $key => $value) {
            ...
            if (!is_dir($path)) {
               ...
               get_dir_contents_gallery_master($path, $results);
               $results[] = $path;
            }
         }
         return $results;
      ...
      if (isset($_REQUEST["param"])) {
         ...
         switch (esc_attr($_REQUEST["param"])) {
            case "wizard_gallery_master":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "gallery_master_check_status")) {
            ...
            case "gallery_master_upload_images":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "gallery_upload_images_nonce")) {
                  ...
                  $get_old_gallery_id = $wpdb->get_var
                      ...
                      $wpdb->prepare
                          (
                          "SELECT old_gallery_id FROM " . gallery_master_meta() . " WHERE meta_id = %d and meta_key = %s", $gallery_id, "gallery_data"
                      )
                  );
                  ...
                  $gallery_images_count = $wpdb->get_var
                      ...
                      $wpdb->prepare
                          (
                          "SELECT count(meta_key) FROM " . gallery_master_meta() . " WHERE meta_id = %d and meta_key = %s", $gallery_id, "image_data"
                      )
                  );
                  if ($file_type == "image") {
                     if ($upload_type != "") {
                        ...
                        if (function_exists("copy")) {
                           ...
                           $obj_image_process_gallery_master->copy_images_gallery_master($src_file[0], GALLERY_MASTER_ORIGINAL_DIR . $fileName);
                           $obj_image_process_gallery_master->copy_images_gallery_master($src_file[0], GALLERY_MASTER_UPLOAD_DIR . $fileName);
                        }
                        $thumbnail_image_name = $obj_image_process_gallery_master->createThumbs_gallery_master($image_name, $image_data);
                     ...
                     $image_meta_data["video_url"] = isset($_REQUEST["embed_video_src"]) ? sanitize_text_field(urldecode($_REQUEST["embed_video_src"])) : "";
                     $embed_video_src = $image_meta_data["video_url"];
                     $image_meta_data["image_title"] = "";
                     $image_meta_data["video_type"] = $video_type;
                     $image_meta_data["image_name"] = isset($_REQUEST["video_url"]) ? sanitize_text_field(urldecode($_REQUEST["video_url"])) : "";
                     $image_meta_data["video_thumb"] = $obj_image_process_gallery_master->online_video_thumb_gallery_master(isset($_REQUEST["video_url"]) ? sanitize_text_field(urldecode($_REQUEST["video_url"])) : "");
                  ...
                  if ($gallery_images_count == 0) {
                     ...
                     $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $gallery_meta_data_insert, $where);
                  }
            ...
            case "gm_get_gallery_id_gallery_master":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "get_gallery_id_gallery_master")) {
            ...
            case "delete_gallery_images":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "gallery_images_delete_nonce")) {
                  ...
                  if (count($delete_image_data) > 0) {
                     $obj_dbHelper_gallery_master->bulk_deleteCommand(gallery_master_meta(), "id", implode(",", $delete_image_data));
                  }
               }
               break;
            ...
            case "update_gallery_data":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "gallery_update_data_nonce")) {
                  ...
                  foreach ($gallery_images_data as $value) {
                     ...
                     $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $update_image_meta_data, $where);
                  }
                  ...
                  $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $gallery_update_data, $where);
               }
               break;
            ...
            case "manage_gallery_module":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "gm_manage_gallery_nonce")) {
                  ...
                  $obj_dbHelper_gallery_master->deleteCommand(gallery_master_meta(), $delete_gallery);
                  $obj_dbHelper_gallery_master->deleteCommand(gallery_master_parent(), $delete_gallery_parent);
               }
               break;
            ...
            case "custom_css_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "custom_css_nonce")) {
                  ...
                  $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $update_custom_css_array, $where);
               }
               break;
            ...
            case "global_options_module":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "global_options_nonce")) {
                  ...
                  $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $global_options_data, $where);
            ...
            case "other_settings_module":
               if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "other_settings_nonce")) {
                  ...
                  $obj_dbHelper_gallery_master->updateCommand(gallery_master_meta(), $update_other_settings_data, $where);
               }
               break;
         }
         die();
