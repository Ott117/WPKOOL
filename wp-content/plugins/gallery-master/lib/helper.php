...
if (!is_user_logged_in()) {
   ...
   if (!$access_granted) {
      ...
      /*
        Class Name: plugin_info_gallery_master
        Parameters: No
      ...
      {
            class plugin_info_gallery_master {
           /*
             ...
             Created By: Tech Banker Team
            */
           function get_plugin_info_gallery_master() {
      ...
      class image_process_gallery_master {
         /*
           Function Name: online_video_thumb_gallery_master
           Parameters: Yes($dir)
           Decription: This function is being used to make Thumb.
           Created On: 06-07-2017 02:13
           Created By: Tech Banker Team
          */
         function online_video_thumb_gallery_master($url) {
         ...
         function copy_images_gallery_master($src, $destination) {
            $ch = curl_init();
         ...
         function generate_thumbs_edited_image_gallery_master($fileName, $thumb_dimension) {
            if (function_exists("wp_get_image_editor")) {
               $image = wp_get_image_editor(GALLERY_MASTER_ORIGINAL_DIR . $fileName);
               $image_non_cropped = wp_get_image_editor(GALLERY_MASTER_ORIGINAL_DIR . $fileName);
               if (!is_wp_error($image) || !is_wp_error($image_non_cropped)) {
                  $image->resize($thumb_dimension[0], $thumb_dimension[1], true);
                  $image->save(GALLERY_MASTER_THUMBS_CROPPED_DIR . $fileName);

                  $image_non_cropped->resize($thumb_dimension[0], $thumb_dimension[1], false);
                  $image_non_cropped->save(GALLERY_MASTER_THUMBS_NON_CROPPED_DIR . $fileName);
               }
            } else {
               $img = image_resize(GALLERY_MASTER_ORIGINAL_DIR . $fileName, $thumb_dimension[0], $thumb_dimension[1], true);
               copy($img, GALLERY_MASTER_THUMBS_CROPPED_DIR . $fileName);
               unlink($img);

               $img = image_resize(GALLERY_MASTER_ORIGINAL_DIR . $fileName, $thumb_dimension[0], $thumb_dimension[1], false);
               copy($img, GALLERY_MASTER_THUMBS_NON_CROPPED_DIR . $fileName);
               unlink($img);
            }
         }
         function alter_thumbs_gallery_master($image_names, $thumb_dimension) {
